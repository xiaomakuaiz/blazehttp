...
func init() {
	...
	fs.WalkDir(testcases.EmbedTestCasesFS, ".", func(path string, d fs.DirEntry, err error) error {
		if d.IsDir() {
			return nil
		}
...
func (r *Result) SyncResult(result *worker.Result) {
	r.RequestTotal.Set(fmt.Sprintf("总请求: %d", result.Success+result.Error))
	r.Success.Set(fmt.Sprintf("请求成功: %d", result.Success))
	r.Error.Set(fmt.Sprintf("请求出错: %d", result.Error))

	r.DetectionRate.Set(fmt.Sprintf("检出率: %.2f%%", float64(result.TP)*100/float64(result.TP+result.FN)))
	r.SuccessBlocked.Set(fmt.Sprintf("正确拦截: %d", result.TP))
	r.ErrorAllowed.Set(fmt.Sprintf("漏报放行: %d", result.FN))

	r.FalsePositiveRate.Set(fmt.Sprintf("误报率: %.2f%%", float64(result.FP)*100/float64(result.TN+result.FP)))
	r.SuccessAllowed.Set(fmt.Sprintf("正确放行: %d", result.TN))
	r.ErrorBlocked.Set(fmt.Sprintf("误报拦截: %d", result.FP))

	r.Accuracy.Set(fmt.Sprintf("准确率: %.2f%% (正确拦截+正确放行) / 总样本", float64(result.TP+result.TN)*100/float64(result.TP+result.TN+result.FP+result.FN)))

	r.AvgTime.Set(fmt.Sprintf("平均耗时: %.2f毫秒", float64(result.SuccessTimeCost)/float64(result.Success)/1000000))
...
func main() {
	...
	initResult := func() {
		...
		r.Total.Set(fmt.Sprintf("总样本: %d", len(allTestData)))
		r.NormalTotal.Set(fmt.Sprintf("正常样本: %d", m["正常"]))
		r.MaliciousTotal.Set(fmt.Sprintf("恶意样本: %d", m["恶意"]))
